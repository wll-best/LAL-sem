...
#sgnet-sem数据集预处理开始------保护我的特殊字符$T$不被tokenize切分
...
def runit(input_path):
    ##改输入文件的格式.先略过第一行，取出tsv文件的每一行的第一列（句子），将句子划分为单词。
    kwords=['$T$','i.e.']
    with open(input_path,"r") as input_file:
        ...
        for (i,line) in enumerate(reader):
            if i==0:
                continue
            sentences.append(multiword_tokenize(line[0],kwords))
    #print(lines)
    #返回的列表形式如[['The', '$T$', 'are', 'not'],[同第一项的形式]]
    return sentences
#预处理结束------
...
def run_parse(args):
    ...
    '''
    sentences = runit(args.input_path)  # 1.改sem数据集输入文件的格式
    ...
    def save_data(syntree_pred, cun):
        ...
        # ----2.将以下两块内容，加上hpsg_list得到包含三种内容的json文件（即sg-net-sem的输入）-------
            ...
            with open(args.output_path_sgnet + appent_string, 'w') as fout:
                for i in range(len(sentences)):
                    ...
                    indict = {'text_tokens': sentences[i],
                              ...
                              'hpsg_list_text': [str(si) for si in (convert_head_to_span([pred_head[i]])[0])]}
                    # convert_所需的输入和生成的是二维数组。形如[[(1, 1), (2, 2), (1, 17)]]
                    data['tag_rep'] = indict
